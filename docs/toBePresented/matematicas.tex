...
% La figura se genera con scripts/plot_cinetica.py (los PDF están gitignorados,
% así que el .tex compila igual sin ella, mostrando una nota en su lugar).
\begin{figure}[h]
\centering
\IfFileExists{cinetica_ctmi.pdf}{%
  \includegraphics[width=0.82\linewidth]{cinetica_ctmi.pdf}%
}{%
  \fbox{\parbox{0.82\linewidth}{\centering\vspace{2ex}\itshape
  Generá la figura con \texttt{python scripts/plot\_cinetica.py} antes de
  compilar.\vspace{2ex}}}%
}
\caption{Curvas de crecimiento CTMI $\mu(T)=\mu_{\mathrm{opt}}\,\gamma_T(T)$ (con
$a_w=1$, $\mathrm{UV}=0$) de las tres especies, generadas con el código real del
motor. Cada curva vale $\mu_{\mathrm{opt}}$ en su óptimo (marcado) y $0$ en sus
cardinales, sin parámetros libres. Reproducible con
\texttt{scripts/plot\_cinetica.py}.}
\end{figure}
...
\section{El orquestador y los dos modos de corrida (ADR-0017)}

Un \textbf{orquestador} único (\textsf{simular}) corre el bucle: por cada tick toma
un \textsf{CampoAmbiental} de un \emph{modo}, le aplica los eventos estocásticos y
avanza el autómata con $\phi$, registrando el historial de las tres poblaciones.
El orquestador \textbf{no sabe de dónde salen los campos}: itera cualquier objeto
que cumpla la interfaz \textsf{ModoSimulacion} ($\mathtt{campos()}\!\to$ iterador de
campos). Así, los dos modos comparten \emph{el mismo bucle} (DRY):
\begin{itemize}[nosep]
  \item \textbf{Modo Sandbox:} campo homogéneo con $T$, $R$, $a_w$ fijados a mano
  (sliders), sin dataset. Explora el espacio de parámetros.
  \item \textbf{Modo Analógico:} una secuencia temporal de campos derivada de los
  datos reales 2025. Para no aplanar el subsuelo (un campo uniforme dejaría el UV de
  superficie esterilizando toda la grilla), cada dato diario se propaga con la
  \emph{estructura espacial} del entorno físico (banda de profundidad en Marte,
  fumarolas en Encelado): el análogo reusa el modelo de campo en vez de duplicarlo.
\end{itemize}
...
Orquestador agnóstico del modo & Sandbox y Analógico comparten un único bucle (DRY, ADR-0017)\\
